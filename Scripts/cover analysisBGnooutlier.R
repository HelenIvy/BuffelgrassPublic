...
#read in data
library(readxl)
Plant_community_cover_exportNOoutlier <- read_excel("Z:/Shared/Field Institute/Research/1 Current Projects/NNP Pennisetum removal experiments/Data/Analysis/BG 2022/Plant community cover exportNOoutlier.xlsx")
View(Plant_community_cover_exportNOoutlier)


DATACoverNO <- Plant_community_cover_exportNOoutlier
...
m1 <- lmer(PECICover ~TREATMENT*YEAR + (1|BLOCKPLOT)+(1|BLOCK), data =  DATACoverNO)
summary (m1)
m2 <- lmer(SQRTPECIcover ~TREATMENT*YEAR + (1|BLOCKPLOT)+(1|BLOCK), data =  DATACoverNO)
summary (m2)
m3 <- lmer(Nativecover ~TREATMENT*YEAR + (1|BLOCKPLOT)+(1|BLOCK), data =  DATACoverNO)
summary (m3)
...
summary (m5)
m6 <- lmer(nativeannualcover  ~TREATMENT*YEAR + (1|BLOCKPLOT)+(1|BLOCK), data =  DATACoverNO)
summary (m6)
m7 <- lmer(SQRTnativeperennialcover ~TREATMENT*YEAR + (1|BLOCKPLOT)+(1|BLOCK), data =  DATACoverNO)
summary (m7)
m8 <- lmer(SQRTnativeannualcover  ~TREATMENT*YEAR + (1|BLOCKPLOT)+(1|BLOCK), data =  DATACoverNO)
summary (m8)
m9 <- lmer(NNcovernoPECI  ~TREATMENT*YEAR + (1|BLOCKPLOT)+(1|BLOCK), data =  DATACoverNO)
summary (m9)
m10 <- lmer(PECIStandingdead  ~TREATMENT*YEAR + (1|BLOCKPLOT)+(1|BLOCK), data =  DATACoverNO)
summary (m10)
